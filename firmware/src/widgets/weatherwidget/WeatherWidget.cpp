...
bool WeatherWidget::getWeatherData() {
    HTTPClient http;
    // Build the request from live settings so location/units can be changed
    // from the web control panel without reflashing.
    String units = settings.getMetric() ? "metric" : "us";
    String url = "https://weather.visualcrossing.com/VisualCrossingWebServices/rest/services/timeline/" +
                 settings.getWeatherLocation() + "/next3days?key=" + weatherApiKey +
                 "&unitGroup=" + units + "&include=days,current&iconSet=icons1&lang=" + LOC_LANG;
    http.begin(url);
