...
bool StockWidget::rebuildFromSettings() {
    String list = settings.getStockList();
    if (list == m_loadedStockList) {
        return false; // no change
    }
    m_loadedStockList = list;
    m_stockCount = 0;

    char stockList[list.length() + 1];
    strcpy(stockList, list.c_str());
...
void StockWidget::update(bool force) {
    // Pick up any change to the tracked-symbol list made from the web panel
    bool listChanged = rebuildFromSettings();
    if (force || listChanged || m_stockDelayPrev == 0 || (millis() - m_stockDelayPrev) >= m_stockDelay) {
        setBusy(true);
        for (int8_t i = 0; i < m_stockCount; i++) {
