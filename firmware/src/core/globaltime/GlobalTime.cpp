...
void GlobalTime::getTimeZoneOffsetFromAPI() {
    HTTPClient http;
    http.begin(String(TIMEZONE_API_URL) + "?key=" + TIMEZONE_API_KEY + "&format=json&fields=gmtOffset,zoneEnd&by=zone&zone=" + settings.getTimezoneLocation());
...
void GlobalTime::forceTimezoneUpdate() {
    // Sentinel that updateTime() treats as "must re-fetch from API"
    m_timeZoneOffset = -1;
}
