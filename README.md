...
# Orbital Orbs — Info Orbs on HackerBox 0129 with a web control panel

This is a fork of [brett.tech's Info Orbs](https://brett.tech/) adapted to run on the
**[HackerBox 0129 "Orbital"](https://www.instructables.com/HackerBox-0129-Orbital/)** hardware
(an **ESP32-S3 SuperMini** driving five GC9A01 round displays), with a **browser-based control
panel and over-the-air (OTA) updates** added on top.

## What this fork adds

- **Web control panel** served from the device (`http://<device-ip>/`) once it's on WiFi:
  - Live status (current widget, brightness, uptime, free memory, firmware version)
  - Live control — switch widgets, brightness slider, auto-rotate timing
  - **Settings editor** — weather location, metric/imperial, stock tickers, timezone, 24-hour
    clock — **saved on the device and applied live, no reflashing**
- **OTA updates** — push new firmware over WiFi, either from the panel's *Firmware update* card
  or with `pio run -t upload --upload-port <device-ip>`. No more USB / BOOT-RESET dance.
- **ESP32-S3 port fixes** — buttons remapped to GPIO 1/2/3 (the original ESP32 map used 26/27,
  which are the S3's flash bus and crash the board), `BUSY_PIN` moved off GPIO 2, and a
  two-slot OTA partition table.

## Quick start

1. Copy `firmware/config/config.h.template` to `firmware/config/config.h` and add your free
   [VisualCrossing](https://www.visualcrossing.com/weather-api) and
   [TimeZoneDB](https://timezonedb.com/register) API keys. (`config.h` is git-ignored.)
2. Build & flash the **first** time over USB (PlatformIO → Upload; on the SuperMini you may need
   download mode: hold BOOT, tap RESET, release BOOT).
3. On boot, join the `Info-Orbs_XX` WiFi network and configure your 2.4 GHz WiFi at `192.168.4.1`.
   The orbs show their IP when connected — open it in a browser for the control panel.
4. All future updates can go **over the air** — no cable needed.

Everything below is the original Info Orbs documentation. Full credit to
[brett.tech](https://brett.tech/) for the project and to
[HackerBoxes](https://hackerboxes.com/) for the Orbital hardware.
